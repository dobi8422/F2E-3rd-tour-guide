      ...
      <ul class="grid grid-cols gap-4">
        ...
        <li v-for="item in list" :key="item.ID">
          <div class="bg-white flex flex-col xl:flex-row p-5 rounded-3xl">
            ...
            <img class="w-80 h-64 rounded-3xl" src="../assets/NoImageFound.png" alt="not found image" v-else>
            <div class="p-5 grid grid-cols-1">
              <h5 class="truncate text-4xl font-bold pb-4">{{ item.Name }}</h5>
              <div class="flex">
                <p class="truncate xl:pt-2.5 mr-1">{{ item.Description ? item.Description : item.DescriptionDetail ? item.DescriptionDetail : '未提供內容描述' }}</p>
