<template>
  <div class="bg-white flex flex-col md:flex-row justify-around lg:justify-between items-center px-10 text-lg h-64 md:h-32 lg:h-20">
    ...
    <div class="flex flex-col lg:flex-grow lg:flex-row lg:justify-around">
      ...
      <div class="rounded-full flex justify-between px-3 py-1 mt-4 lg:mt-0 border-2" :class="searchPage ? `border-${props.theme}-800 bg-${props.theme}-800 text-white`: `border-${props.theme}-400 text-gray-400`">
        <button :class="searchPage ? 'text-red-500' : ''" @click="cancelSearch"><i class="far fa-times-circle"/></button>
        <input class="text-center outline-none w-32" :class="searchPage ? `bg-${props.theme}-800 placeholder-white`: ''" type="text" :placeholder="searchPage ? keyword : '景點關鍵字'" v-model="keyword" @keyup.enter="search" :readonly="searchPage ? searchPage : false">
        <div>
          <select class="cursor-pointer text-center rounded-full outline-none text-white mr-2 px-1" :class="`bg-${props.theme}-400`" v-model="county">
